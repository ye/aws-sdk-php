...
class ClientResolver
{
     ...
     *   value. The function is provided the array of options and the client
     *   object client object being built. The function is expected to return
     *   the default value.
     * - doc: (string) The argument documentation string.
     * - fn: (callable) Function used to apply the argument. The function
     *   accepts the provided value, array of arguments by reference, and the
     *   client object being created.
    ...
    public static function getDefaultArguments()
    {
        ...
        return $args = [
            ...
            'retry_logger' => [
                'type'  => 'value',
                'value' => ['string', 'Psr\Log\LoggerInterface'],
            ...
            'validate' => [
                ...
                'fn'      => function ($value, array &$args, AwsClientInterface $client) {
                    if ($value === true) {
            ...
            'debug' => [
                ...
                'fn'    => function ($value, $_, AwsClientInterface $client) {
                    if ($value !== false) {
    ...
    /**
     * Resolves the configuration options and applies them to a client.
     *
     * @param AwsClientInterface $client Client to resolve.
     ...
     */
    public function resolve(AwsClientInterface $client, array $args)
    {
        $args['config'] = [];
            ...
            if (!array_key_exists($key, $args)) {
                if (isset($a['default'])) {
                    ...
                    if (!is_callable($a['default'])) {
                        ...
                        $fn = $a['default'];
                        $args[$key] = $fn($args, $client);
                    }
                } elseif (!empty($a['required'])) {
                    // Allows custom error messages for missing values.
                    throw new IAE(
                        is_callable($a['required'])
                            ? call_user_func($a['required'], $args)
    ...
    /** @internal */
    public static function _apply_retries($value, array &$args, AwsClientInterface $client)
    {
        if ($value) {
